...
void loadDoorPassword() {
  ...
  if (Firebase.RTDB.getString(&fbdo, path)) {
    ...
    if (fetched.length() > 0) {
      correctPassword = fetched;
      Serial.println(">> Password loaded: " + correctPassword);
    }
  } else {
    Serial.println("!! Load password failed (giu cache cu): " + fbdo.errorReason());
  }
}
...
void openDoor(String source) {
  lastDoorStatus = "ON";  // FIX: chong trigger lai trong vong loop tiep theo
...
void handleButtons() {
  ...
  if (digitalRead(BTN_BEDROOM) == LOW) {
    ...
    if (digitalRead(BTN_BEDROOM) == LOW) {
      ...
      // FIX: them timeout tranh block loop mai mai neu nut bi ket
...
void setup() {
  ...
  time_t t = 0;
  int ntpRetry = 0;
  while (t < 100000 && ntpRetry < 20) {
    time(&t);
    delay(500);
    Serial.print(".");
    ntpRetry++;
  }
  if (t > 100000) Serial.println(" NTP OK");
  else            Serial.println(" NTP timeout (bo qua)");
